...
const printColor =
  ...
  (...args: unknown[]) => {
    ...
    // Utiliser try/catch pour éviter les erreurs avec chalk
    try {
      // Vérifier si les méthodes hex et bgHex sont disponibles
      if (typeof chalk.hex !== "function" || typeof chalk.bgHex !== "function") {
        // Si les méthodes spécifiques ne sont pas disponibles, retourner le texte brut
        return data
      }

      // Utiliser les méthodes hex si disponibles
      if (bg && text2) return chalk.bgHex(bg).hex(text2)(data)
      if (bg) return chalk.bgHex(bg)(data)
      if (text2) return chalk.hex(text2)(data)
      return data
    } catch (error) {
      // En cas d'erreur avec chalk, retourner simplement le texte brut
      return data
    }
  }
